...
export async function getCategories(): Promise<Category[]> {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error fetching categories (retries left: ${retries - 1}):`, error.message);
      retries--;
      if (retries === 0) return [];
...
export async function getMenuItems(): Promise<MenuItem[]> {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error fetching menu items (retries left: ${retries - 1}):`, error.message);
      retries--;
      if (retries === 0) return [];
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
...
export async function getCustomers(): Promise<Customer[]> {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error fetching customers (retries left: ${retries - 1}):`, error.message);
      retries--;
      if (retries === 0) return [];
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
...
export async function getBookings(): Promise<Booking[]> {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error fetching bookings (retries left: ${retries - 1}):`, error.message);
      retries--;
      if (retries === 0) return [];
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
...
export async function getAdmins(): Promise<Admin[]> {
  ...
  while (retries > 0) {
    try {
      ...
      console.error(`Error fetching admins (retries left: ${retries - 1}):`, error.message);
      retries--;
      if (retries === 0) return [];
      await new Promise(resolve => setTimeout(resolve, 1000));
    }
