...
const CHART_CONFIG = {
  ...
  popularity: { label: "Popularity", color: "hsl(var(--chart-1))" },
  prices: { label: "Prices", color: "hsl(var(--chart-3))" },
...
export function AnalyticsTab({ categories, menuItems, rooms, bookings }: AnalyticsTabProps) {
  ...
  const menuPriceDistribution = useMemo(() => {
    const bins = [
      { name: '0-10 Birr', count: 0, fill: 'hsl(var(--chart-1))' },
      { name: '10-15 Birr', count: 0, fill: 'hsl(var(--chart-2))' },
      { name: '15-20 Birr', count: 0, fill: 'hsl(var(--chart-3))' },
      { name: '20+ Birr', count: 0, fill: 'hsl(var(--chart-4))' },
    ];
    menuItems.forEach(item => {
            ...
            <ChartContainer config={CHART_CONFIG}>
              <BarChart data={roomBookingCounts} margin={{ top: 20, right: 20, left: -10, bottom: 0 }}>
                ...
                <Bar dataKey="bookings" radius={4}>
                  {roomBookingCounts.map((entry, index) => (
              ...
              <ChartContainer config={CHART_CONFIG}>
                <BarChart data={categoryItemCounts} margin={{ top: 20, right: 20, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="items" radius={4}>
                    {categoryItemCounts.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.fill} />
                    ))}
                  </Bar>
                </BarChart>
            ...
            <CardContent>
              <ChartContainer config={CHART_CONFIG}>
                <BarChart data={menuPriceDistribution} margin={{ top: 20, right: 20, left: -10, bottom: 0 }}>
                  ...
                  <YAxis />
                  <Tooltip cursor={false} content={<ChartTooltipContent />} />
                  <Bar dataKey="count" radius={4} name="Items">
                    {menuPriceDistribution.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.fill} />
                    ))}
                  </Bar>
                </BarChart>
